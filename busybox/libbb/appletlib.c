...
# endif
	...
	/* PATCH: If launched without arguments, start ash shell directly */
	if (!argv[1]) {
		/* Declare ash_main function */
		extern int ash_main(int argc, char **argv) MAIN_EXTERNALLY_VISIBLE;
		/* Set custom prompt dynamically based on USER and HOSTNAME env vars */
		const char *user = getenv("USER");
		if (!user || !user[0]) user = "root";
		const char *host = getenv("HOSTNAME");
		if (!host || !host[0]) host = "localhost";
		char *ps1 = malloc(strlen(user) + strlen(host) + 16);
		if (ps1) {
			sprintf(ps1, "PS1=%s@%s:$PWD%c ", user, host, (strcmp(user, "root") == 0) ? '#' : '$');
			putenv(ps1);
		}
